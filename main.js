...
(function () {
  ...
  function setupHeroVideoSound() {
    ...
    function autoMuteAfterFirstPlay() {
      // Video is looped so 'ended' never fires; use timeout instead
      const muteNow = () => {
        video.muted = true;
        localStorage.setItem('tf_hero_audio_done', '1');
        if (soundBtn) soundBtn.style.display = 'none';
      };
      // FIX: looped videos never fire 'ended'; rely on the 10 s timeout only
  ...
  /* ─── CONTACT FORM (Google Sheets + Email + WhatsApp) ─── */
  window.submitContactForm = async function (evt) {
    ...
    const data = {
      ...
      localita:  form.localita?.value?.trim()  || '',
    ...
    if (!data.nome || !data.email || !data.telefono) {
      if (errorMsg) {
        errorMsg.textContent = currentLang === 'it'
          ? '⚠️ Compila tutti i campi obbligatori (Nome, Email, Telefono).'
    ...
    // Build WhatsApp message with both numbers
    const waMsg = encodeURIComponent(
      `Ciao TF ENERGY SRL! Ho appena inviato una richiesta dal vostro sito.\n\n` +
      `👤 Nome: ${data.nome}\n` +
      `📧 Email: ${data.email}\n` +
      `📞 Tel: ${data.telefono}\n` +
      `📍 Località: ${data.localita || 'N/D'}\n` +
      `⚡ Prodotto: ${data.prodotto || 'N/D'}\n` +
      `💬 Messaggio: ${data.messaggio || 'N/D'}`
    );
    // FIX: use the same primary WhatsApp number used everywhere else on the site
    const waURL = `https://wa.me/393792620610?text=${waMsg}`;

    try {
      // ── 1. Send to Google Sheet + Email via Vercel serverless ──
      let sheetOk = false;
      try {
        ...
        console.warn('⚠️ /api/submit failed, using fallback:', apiErr.message);
        // Fallback: direct Apps Script POST
        const fd = new URLSearchParams();
        Object.entries(data).forEach(([k, v]) => fd.append(k, v));
        await fetch(
          'https://script.google.com/macros/s/AKfycbwhv18aTF6TaO6HPtw5WZdaa8kdqPG4pq3kYQAhcbver4MtY-pMsY7pTji64oJFqrg4/exec',
      ...
      // ── 3. Show success with countdown ──
      if (successMsg) {
        let secs = 3;
        successMsg.innerHTML = currentLang === 'it'
          ? `✅ Richiesta inviata! Una copia è stata inviata a <strong>info@tfenergy.it</strong>.<br>Apertura WhatsApp in <strong id="wa-countdown">${secs}</strong> secondi…`
          : `✅ Request sent! A copy was sent to <strong>info@tfenergy.it</strong>.<br>Opening WhatsApp in <strong id="wa-countdown">${secs}</strong> seconds…`;
      ...
      if (successMsg) {
        successMsg.innerHTML = currentLang === 'it'
          ? `✅ Richiesta salvata! Apertura WhatsApp…`
      ...
      if (submitBtn) {
        ...
        submitBtn.textContent = currentLang === 'it' ? 'Invia Richiesta' : 'Send Request';
      }
    }
  };
  ...
  /* FIX: use SHA-256 (Web Crypto API) instead of insecure base64 */
  async function hashPass(pass) {
    const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(pass));
    return Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, '0')).join('');
  }

  window.adminLogin = async function (evt) {
    if (evt) evt.preventDefault();
    const user = document.getElementById('admin-user')?.value;
    const pass = document.getElementById('admin-pass')?.value;
    const admins = JSON.parse(localStorage.getItem('tf_admins') || '[]');
    const hashed = await hashPass(pass);
    // Support legacy btoa accounts during transition
    const found = admins.find(a => a.email === user && (a.pass === hashed || a.pass === btoa(pass)));
    if (found) {
      // Upgrade legacy btoa hash to SHA-256 on next login
      if (found.pass === btoa(pass)) {
        found.pass = hashed;
  ...
  window.adminRegister = async function (evt) {
    if (evt) evt.preventDefault();
    ...
    const hashed = await hashPass(pass);
    admins.push({ email, pass: hashed, role: 'admin', created: new Date().toISOString() });
    localStorage.setItem('tf_admins', JSON.stringify(admins));
